	...
	if (method == 0) {
		...
		for (size_t i = 0; i < 7; ++i) {
			...
			for (size_t j = 0; j < 14; ++j) {
				...
				gf.reset_counters();
				std::vector<unsigned> a(2 * q, 1), b(2 * q, 1), c(2 * q);
				for (size_t k = 0; k < 2*q; ++k) {
					b[k] = ((((k + j) + 12) * 3) % 7) * 100 % (q - 2);
					a[k] = ((((k + j) + 47) * 5) % 9) * 97 % (q - 2);
				}
		...
		for (size_t i = 0; i < 6; ++i) {
			...
			std::vector<unsigned> field_generators = { 0xb, 0x13, 0x25, 0x43, 0x83, 0x11d, 0x211, 0x409, 0x805, 0x1053, 0x201b, 0x4143, 0x8003, 0x1100b };
			//// 2 2
			for (size_t i = 0; i < 8/*field_sizes.size()*/; ++i) {
...
int main(int argc, char* argv[])
{
	...
	benchmark_multiplication("int_half_max_unimproved", 4);
	return 0;

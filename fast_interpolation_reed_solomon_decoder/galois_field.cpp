...
void galois_field::init() {
	...
	for (size_t i = 0; i < _schonhage_dft_tmp.size(); ++i) {
		...
		_schonhage_dft_results_tmp[i].resize(3);
...
std::vector<unsigned>& galois_field::fast_poly_multiplication(std::vector<unsigned>& a, std::vector<unsigned>& b, std::vector<unsigned>& dst, unsigned length) {
	...
	//if (length < 256 || length > _q / 2) {
	size_t len = 1 << (sizeof(unsigned) * 8 - std::countl_zero(length - 1));
	karatsuba_multiplication(a, b, _multiplication_result_tmp, len, 0);
	...
	//	gao_mateer_fft(a, _karatsuba_tmp[0][0], m);
	//	gao_mateer_fft(b, _karatsuba_tmp[0][1], m);
	//	for (size_t i = 0; i < (1 << m); ++i) {
	//		_karatsuba_tmp[0][2][i] = multiply(_karatsuba_tmp[0][0][i], _karatsuba_tmp[0][1][i]);
	//	}
	//	//print_poly(_karatsuba_tmp[0][2]);
	//	gao_mateer_ifft(_karatsuba_tmp[0][2], _multiplication_result_tmp, m);
	//	//print_poly(_multiplication_result_tmp);
	//} 
	 /*else {
	...
	/*if ((length <= 512 && (length > _n / 2 || length < _n / 4)) || length >= 64) {
		...
		std::fill(_multiplication_result_tmp.begin(), _multiplication_result_tmp.end(), 0);
		exDFT(a, _karatsuba_tmp[0][0]);
		exDFT(b, _karatsuba_tmp[0][1]);
		for (size_t i = 0; i < _q; ++i) {
			_karatsuba_tmp[0][2][i] = multiply(_karatsuba_tmp[0][0][i], _karatsuba_tmp[0][1][i]);
		}

		exIDFT(_karatsuba_tmp[0][2], _multiplication_result_tmp);
		for (size_t i = 0; i < 3; ++i) {
			std::fill(_karatsuba_tmp[0][i].begin(), _karatsuba_tmp[0][i].begin() + _n, 0);
		}
...
std::vector<unsigned>& galois_field::SCHONHAGE_CONVOLUTION(std::vector<unsigned>& a, std::vector<unsigned>& b, std::vector<unsigned>& dst, unsigned length, unsigned root, unsigned m, unsigned level) {
	...
	for (size_t i = 0; i < length; ++i) {
		...
		//fast_poly_multiplication(tmp[2], tmp[3], tmp[4], 4 * m);
		SCHONHAGE_STRASSEN_FFT(tmp[2], tmp[3], tmp[4], m, level + 1);
		//std::cout << degree(tmp[4]) << " " << block_size << "\n";
...
std::vector<unsigned>& galois_field::karatsuba_multiplication(std::vector<unsigned>& a, std::vector<unsigned>& b, std::vector<unsigned>& dst, unsigned length, unsigned lvl) {
	auto& tmp = _karatsuba_tmp[lvl];
	std::fill(dst.begin(), dst.end(), 0);
	...
	if (length <= _q / 2 && std::min(std::max(deg_a, deg_b) + 1, length) >= 256) {
		...
		unsigned m = sizeof(unsigned) * 8 - std::countl_zero(std::min(std::max(deg_a, deg_b) + 1, length) - 1) + 1;
		std::fill(_multiplication_result_tmp.begin(), _multiplication_result_tmp.end(), 0);
	...
	karatsuba_multiplication(tmp[0], tmp[2], tmp[4], k, lvl + 1); // F0*G0
	karatsuba_multiplication(tmp[1], tmp[3], tmp[5], k, lvl + 1); // F1*G1
	add_poly_to(tmp[0], tmp[1], 0, k);
	add_poly_to(tmp[2], tmp[3], 0, k);
	karatsuba_multiplication(tmp[0], tmp[2], tmp[1], k, lvl + 1); // (F0 + F1)*(G0 + G1)
...
std::vector<unsigned>& galois_field::taylor_expansion(std::vector<unsigned>& src, std::vector<unsigned>& dst, unsigned n, unsigned t) {
	...
	std::fill(tmp[0].begin(), tmp[0].end(), 0);
	taylor_expansion(tmp[1], tmp[0], p, t);
	// (g0 + g1x) + (g2 + g3x)(x + x^2) + (g4 + g5x)(x + x^2)^2..
	// do result
	std::copy(tmp[2].begin(), tmp[2].begin() + p, dst.begin());
	std::copy(tmp[0].begin(), tmp[0].begin() + p, dst.begin() + p);
	return dst;
}
